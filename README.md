...
### Platforms
Linux only for now, more to follow.
...
5. Linux only: install `xclip` package for working with clipboard.
...
### Notes
1. cpwd is fully compatibile with latest npwd 1.0.6 and offers the same functional. The only real differences are that cpwd is written on C while npwd is written in JavaScript; cpwd depends only on [xclip] (http://xclip.sourceforge.net) on Linux while npwd also depends on [Node.JS] (https://nodejs.org) and [npm] (https://www.npmjs.com).
